...
const CustomerTransactions = () => {
    ...
    const fetchTransactions = async () => {
        try {
            const token = localStorage.getItem('token');
            const response = await axios.get(API_BASE_URL + '/customer/my-transactions', {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const handleDeposit = async () => {
        ...
        try {
            const token = localStorage.getItem('token');
            const res = await axios.post(`${API_BASE_URL}/customer/deposit/${selectedAccount}/${depositAmount}`, {}, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const handleWithdraw = async () => {
        ...
        try {
            const token = localStorage.getItem('token');
            const res = await axios.post(`${API_BASE_URL}/customer/withdraw/${selectedAccount}/${withdrawAmount}`, {}, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const handleTransfer = async () => {
        ...
        try {
            const token = localStorage.getItem('token');
            const res = await axios.post(`${API_BASE_URL}/customer/transfer/${fromAccount}/${toAccount}/${amount}`, {}, {
                headers: { Authorization: `Bearer ${token}` }
            });
